...
var _extraFeatures = __webpack_require__(25);

var _extraFeatures2 = _interopRequireDefault(_extraFeatures);
...
var CardLayout = function () {
	...
	_createClass(CardLayout, [{
		...
		key: 'onLayoutComplete',
		value: function onLayoutComplete($instance) {
			$instance.on('layoutComplete', function (items) {
				if (items.length) {
					items.forEach(function (item) {
						var $carousel = $(item.element).find(".carousel");
						if ($carousel.length) {
							// activate carousel
							$carousel.carousel();
						}
