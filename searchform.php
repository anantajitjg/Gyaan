...
<form role="search" method="get" class="search-form gyaan-search-form form-inline needs-validation" action="<?php echo esc_url( home_url( '/' ) ); ?>" novalidate>
	<div class="input-group">
		<label class="screen-reader-text" for="<?php echo $id; ?>"><?php echo _x( 'Search for:', 'label', 'gyaan' ); ?></label>
		<input type="search" id="<?php echo $id; ?>" class="search-field form-control" placeholder="<?php echo esc_attr_x( 'Search', 'placeholder', 'gyaan' ); ?>" value="<?php echo get_search_query(); ?>" name="s" required />
